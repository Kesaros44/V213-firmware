...
#define _MBASE 1
#define _WMFN1 2
// _WMFN2(예비 빈 레이어)는 Vial 빌드에서는 제외함 — atmega32u4 1KB EEPROM에
// 4레이어분 다이나믹 키맵이 다 안 들어가서 실제로 쓰이지 않는 레이어를 뺌.
// (DYNAMIC_KEYMAP_LAYER_COUNT 3으로 keymaps/vial/config.h에서 맞춰둠)
...
const uint16_t PROGMEM keymaps[][MATRIX_ROWS][MATRIX_COLS] = {
    ...
    [_MBASE] = LAYOUT(
        ...
        KC_P4,   KC_P5,   KC_P6,   KC_CAPS, KC_A,    KC_S,    KC_D,    KC_F,    KC_G,                              KC_H,    KC_J,    KC_K,    KC_L,    KC_SCLN, KC_QUOT, KC_ENT,  
        KC_P1,   KC_P2,   KC_P3,   KC_PENT, KC_LSFT, KC_Z,    KC_X,    KC_C,    KC_V,    KC_B,                     KC_B,    KC_N,    KC_M,    KC_COMM, KC_DOT,  KC_SLSH, KC_RSFT, KC_UP,  
        KC_P0,   KC_PDOT, KC_LCTL, KC_LOPT, KC_LCMD, KC_SPC,                                                       KC_SPC,  KC_CAPS, MO(2),   KC_RCMD, KC_ROPT, KC_RCTL, KC_LEFT, KC_DOWN, KC_RGHT  
    ),
...
bool encoder_update_user(uint8_t index, bool clockwise) {
    tap_code_delay(clockwise ? KC_VOLU : KC_VOLD, 10);  // 10ms 지연: macOS 인식 안정
    return false;  // QMK 기본 볼륨 처리 중복 방지 (true면 한 칸에 두 번 전송됨)
}
